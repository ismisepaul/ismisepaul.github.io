...
modelling. She also recommends to use [Adams four questions](https://github.com/adamshostack/4QuestionFrame) but also 
ensure you have capacity. My key takeaway from Tanya's talk is the security team needs to have good soft skills and be
empathic. She opened with the example of where the security team pushes out a best practice like storing all their 
secrets in a secret management tool but when she went to talk to the teams why they're not following the published best
practice the development teams explained they weren't allowed to buy one.

There's a couple of tips she had for creating threat models
...
- Get feedback all the time - ask what concerns them, what they wish security could provide nad what is missing 
...
By having regular touch points with people it not only helps the program but Tanya said it's also a way to measure the 
...
The reason behind the OWASP Serverless Top 10 is simply not everything that applies to web vulnerabilities is in the 
cloud. Serverless, as Tal describes in his talk, is something that's not a monolith and not even a microservice but 
really tiny chunks of code where teams work hyper agile, code is shipped to prod once a day and everything is 
automated. When developing everything is done bottom up where its the developers who take the decision to deploy to prod.

Serverless architecture

- Each resource needs its own security
- Everything is a lamda function that runs code
...
- You develop in a read only enviroment with the exception of `/tmp`
- It's not wired to Internet 
- Data is temporary 
- All Code resides in environment 
...
multiple entry points tigger events and usually there's no continuous flow of data. As an example Tal used the example 
of an application that takes user's input which sends an email to a manager who has to send a reply to trigger the 
lambda function. An attacker could bypass the input by sending a malicious email directly to the SES to trigger the same
lambda function and bypassing authentication. 
...
of these functions are misconfiguration in this way.

Using the lamda code snippet below with a the database configured like 
...
Set lease privileges such as below
...
terminator type risk but the biggest risk is that AI is demonstrating that it can be manipulative and gave an [example](https://fortune.com/2023/02/17/microsoft-chatgpt-bing-romantic-love/)
...
and spoke about from a legal perspective things are going to get messy really fast. If the code is AI generated who
owns the intellectual property? If its assisted its a grey area but if generated solely by the AI then it belongs to
the creator of the AI. If someone on your team writes code with AI and embeds it in your code. Would you know and who 
owns it?

She went into the attractiveness for a business of AI vs a human who is developing software bearing in mind that quaility
needs to be a factor and not just speed. She highlighted the following

- How long it takes to hire the right person 1 month, 3 months, 6 months after job posting 
- Is that person able to fit into the budget and can do their job
- How manu features can they develop and how many bugs can they fix
- How long does it take for them to be affective - 1 month / 3 months
...
vulnerabilities. Not only security risks but also ethical issues and biases. There's also attacks against the model
itself like messing with the inputs, data poisoning the training data. She highlighted that in Asian countries' 
security testing is not as mature as in Europe and now that anyone can write code freelancers who sell their services
to write code can now just use an AI and sell on what the AI wrote. Yes there's regulations in place but places like 
Tunisia sells coding services into Europe, will this be AI generated without Europeans knowing? 

She closed out by saying AI was here to stay and it'll transform software development where the real value in her 
opinion is through testing and automation (she takes a shot at the testers here saying it's a boring job and let's leave
...
Jakub gave a talk which was based around the tech stack used in his company. I'm always hesitant when I hear product
security team's building their own solutions as these usually become essential to the business. Embedding into the 
developers' workflow where you can be a bottleneck and delay things if the piece of software you've built has a bug in it.
There's also the issue of the product security team owning the code. What happens when people move from the team or 
leave the company. Is it possible for someone else to take over that role. Most security people aren't full blown 
software developers so it may be difficult to sustain such a program. 

The idea and solution the built was interesting but it would have been great if it were open sourced, something 
Jakub aluded to he would be happy about and wanted. The program they build did the following

• Each pull request has an associated threat model
• It takes less than 5 minutes to complete a pull request
• Threat modeling is done by the developers
• Threats and mitigations are stored in a database
• Exact line references for security mechanisms can be queried
• Product Security is directly involved only in when requested by the developement teams (10% - 20%)
...
- If the risk is medium then threat modeling is by the engineers
...
implemented it his team wasn't helping them. The pushed the "Threat model every story" which wasn't received well so 
...
People also need to understand the fact that threat modelling is a conceptional process and in his opinion the human will 
...
Bad metics to use (you may need to go back and accept the risk so numbers can be skewed)
- No of threats identifiyed
...
2. Understand, explain, imagine what could possibly go wrong
...
5. Get everything in one place - its not clear what to do with the results
6. Get people to act on the results 

Scale

He says this one is going to hurt! You'll notice the difference between teams really shows and may not work for all teams
...
* Ask if they've got any value as people will just try get you off their backs
